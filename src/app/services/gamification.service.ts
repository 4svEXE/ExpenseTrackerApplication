...
// Add Advice Events (Use constants for variability)
const adviceCharacters = [
  'Фінансовий Гуру', 'Скупий Лицар', 'Вуличний Торговець', 'Привид Багатого Дядька', 'Капітан Очевидність',
  'Банкір-відлюдник', 'Економна Відьма', 'Золотий Гусак', 'Вовк з Говерла-стріт'
];

const usefulAdvice = [
  'Завжди відкладай 10% доходу, перш ніж щось купувати.',
  'Складний відсоток — це восьме чудо світу. Починай інвестувати рано.',
  'Твоя машина — це пасив, а не актив, якщо вона не приносить гроші.',
  'Найкраща інвестиція — це твоя освіта та здоров’я.',
  'Уникай споживчих кредитів на речі, що швидко втрачають вартість.',
  'Створи фінансову подушку на 3-6 місяців життя.',
  'Диверсифікуй свої заощадження: не тримай всі яйця в одному кошику.',
  'Ведення бюджету — це не обмеження, а свобода розуміти свої можливості.',
  'Інфляція з’їдає готівку під матрацом. Купуй активи.',
  'Спершу плати собі (в заощадження), а потім рахункам.'
];

const funnyAdvice = [
  'Щоб мати гроші, треба їх не витрачати. Геніально, правда?',
  'Якщо купити щось зі знижкою 50%, ти не заощадив 50%, ти витратив 50%.',
  'Найкращий спосіб подвоїти гроші — скласти купюру навпіл.',
  'Гроші не приносять щастя, але плакати в Lamborghini зручніше.',
  'Заощаджуй на воді — пий каву в гостях.',
  'Хочеш стати мільйонером? Почни як мільярдер і купи авіакомпанію.',
  'Якщо в тебе немає грошей, ти не можеш їх втратити. Безпека!',
  'Порада дня: не будь бідним. Це дуже дорого.',
  'Ніколи не купуй те, що не можеш дозволити собі двічі.',
  'Мій гаманець як цибуля: коли я його відкриваю, я плачу.'
];

for (let i = 201; i <= 300; i++) {
  const char = adviceCharacters[Math.floor(Math.random() * adviceCharacters.length)];
  const isFunny = Math.random() > 0.5;
  const advice = isFunny 
    ? funnyAdvice[Math.floor(Math.random() * funnyAdvice.length)]
    : usefulAdvice[Math.floor(Math.random() * usefulAdvice.length)];

  FANTASY_EVENTS.push({
    id: `advice-${i}`,
    text: `${char} зупиняє вас на шляху і хоче дати безкоштовну фінансову пораду. Слухати?`,
    iconUrl: '',
    choices: [
      {
        text: 'Слухати (Безкоштовно)',
        cost: 0,
        outcomes: [
          { text: `"${advice}" — Ви почули це і замислились.`, reward: 2, probability: 0.1 }, // Small reward for learning
          { text: `"${advice}" — Ну, принаймні це було цікаво.`, reward: 0, probability: 0.9 }
        ]
      },
      {
